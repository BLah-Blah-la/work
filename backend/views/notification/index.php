...
<div class="page-header">
    <div class="buttons">
        <a class="btn btn-danger" href="<?= Url::toRoute(['/notification/delete-all']) ?>"><?= Yii::t('modules/notifications', 'Delete all'); ?></a>
        <a class="btn btn-secondary" href="<?= Url::toRoute(['/notification/read-all']) ?>"><?= Yii::t('modules/notifications', 'Mark all as read'); ?></a>
    </div>

    <h1>
        <span class="icon icon-bell"></span>
        <a href="<?= Url::to(['/notification/manage']) ?>"><?= Yii::t('modules/notifications', 'Notifications') ?></a>
    </h1>
</div>
    ...
    <ul id="notifications-items">
        ...
        <li class="notification-item<?php if($notif['read']): ?> read<?php endif; ?>" data-id="<?= $notif['id']; ?>" data-key="<?= $notif['key']; ?>">
            <a href="<?= $notif['url'] ?>">
                ...
                <span class="message"><?= Html::encode(strip_tags(($notif['message']))); ?></span>
            </a>
